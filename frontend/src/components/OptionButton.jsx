...
const OptionButton = ({ name, buttonArray, onClick, flag }) => {
  ...
  return (
    <div>   
        <button disabled className="w-[10%] py-2 px-4 font-semibold rounded-s-lg bg-slate-600 text-slate-100 border-s-4 border-y-4">
            {name}
        </button>
        {buttonArray.map(({id, content, className, disabled, status}) => (
            ...
            className={`px-4 py-2 font-semibold hover:bg-slate-200 duration-200 
                ...
                ${status === "empty" && flag == content ? "bg-red-400 text-slate-100 hover:text-red-400" : status === "empty" ? "text-red-400" : ""}
